...
path_pqt <- "C:/Users/kanto.fiaferana/Documents/tables_parquet/"
path_sas <- "C:/Users/kanto.fiaferana/Documents/My SAS Files/9.4/"

memory.limit(size = 18000)

# liste des établissements ouverts (ref_etab) ----
ref_etab <- read_sas(paste0(path_sas, "ref_etab.sas7bdat")) %>% 
  clean_names() %>% 
...
if(!exists("cellule")){
  cellule <- read_sas(paste0(path_sas, "t_dwh_h_cellule.sas7bdat")) %>% 
    clean_names() %>% 
    left_join(ref_etab) }
...
if(!exists("situ_penit")){
  situ_penit  <- read_parquet(str_glue("{path_pqt}t_dwh_h_situ_penit.parquet"),
...
if(!exists("mouv_temp")){
  mouv_temp <- read_parquet(str_glue("{path_pqt}t_dwh_f_mouvement.parquet"),
