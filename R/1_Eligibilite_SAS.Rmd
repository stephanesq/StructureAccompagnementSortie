...
library(haven)
library(tidylog)
options(tidylog.message.colors = "green")
```
...
path_dwh = paste0(here::here(),"/Donnees/DWH/")
path_ref = paste0(here::here(),"/Donnees/Referentiel/")
...
```{r}
T_DWH_H_SITU_PENALE <- read_parquet(paste0(path_dwh,"t_dwh_h_situ_penale.parquet"))
...
T_DWH_H_SITU_PENALE <- T_DWH_H_SITU_PENALE %>% 
  ...
  filter(!is.na(DT_LIBE_PREV))

# T_DWH_H_SITU_PENALE_2 <- read_parquet(paste0(path_dwh,"t_dwh_h_situ_penale.parquet"))
# info_libprev <- T_DWH_H_SITU_PENALE_2 %>%  
#   select(NM_ECROU_INIT,DT_DEBUT_SITU_PENALE,DT_LIBE_PREV, TP_SOURCE) %>% 
#   mutate(ANNEE = year(DT_DEBUT_SITU_PENALE),
#          INDIC_LIBPREV = if_else(is.na(DT_LIBE_PREV),0,1)) %>% 
#   arrange(NM_ECROU_INIT,DT_DEBUT_SITU_PENALE)
# 
# table(info_libprev$ANNEE,info_libprev$INDIC_LIBPREV, useNA = "ifany")
